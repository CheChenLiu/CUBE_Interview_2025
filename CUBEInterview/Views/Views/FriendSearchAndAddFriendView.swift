//
//  FriendSearchAndAddFriendView.swift
//  CUBEInterview
//
...
protocol FriendSearchAndAddFriendViewDelegate: AnyObject {
    func focusSearch()
    func outFocusSearch()
}

class FriendSearchAndAddFriendView: UIView {
    weak var delegate: FriendSearchAndAddFriendViewDelegate?
    ...
    @objc private func refreshData() {
        viewModel.parseRequestModel()
    }
}

extension FriendSearchAndAddFriendView: UITableViewDataSource {
...
extension FriendSearchAndAddFriendView: UITableViewDelegate {
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return UITableView.automaticDimension
    }
}

extension FriendSearchAndAddFriendView: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        print("0401 textFieldDidBeginEditing")
        delegate?.focusSearch()
    }
    
    func textFieldDidChangeSelection(_ textField: UITextField) {
        guard let text = textField.text else { return }
        viewModel.filterFriendsAtAll(by: text)
        print("0401 textFieldDidChangeSelection")
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        viewModel.filterFriendsAtAll(by: text)
        print("0401 textFieldDidEndEditing")
    }
}
